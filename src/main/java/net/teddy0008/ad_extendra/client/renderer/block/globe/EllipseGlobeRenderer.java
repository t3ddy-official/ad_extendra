...
public class EllipseGlobeRenderer implements BlockEntityRenderer<EllipseGlobeBlockEntity> {
    public EllipseGlobeRenderer(BlockEntityRendererProvider.Context ctx) {
    }

    public void render(EllipseGlobeBlockEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        public void renderByItem(ItemStack stack, ItemDisplayContext itemDisplayContext, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
            ...
            if (minecraft.level != null) {
                ...
                try {
                    ...
                    Minecraft.getInstance().getBlockRenderer().getModelRenderer().renderModel(poseStack.last(), buffer.getBuffer(Sheets.cutoutBlockSheet()), state, blockModel, 1.0F, 1.0F, 1.0F, packedLight, packedOverlay);
                    EllipseGlobeRenderer.render(state, yaw, poseStack, buffer, packedLight, packedOverlay);
